...
author: "Report prepared by Naftali"
date: "2024-02-24"
output:
  html_document:
    toc: true
...
# read in the data 
cust_sleep_complete <- readRDS("C:/Users/hp/Downloads/Fitness Project-2022/data/cust_sleep_complete.Rds")

cust_dev_info <- readRDS("C:/Users/hp/Downloads/Fitness Project-2022/data/cust_dev_info.Rds")
...
than customers of MINGAR's other lines have been drawn to the new
affordable product lines "Active" and "Advance."
...
Canadian dollar increase in that customer's median region income (see
...
-   Naftali didn't have direct access to client data like income.
    Instead, consumer income was calculated based on the area's median
    income via postal code.
...
for MINGAR's two cheap lines ('Active' and 'Advance,') Naftali examines
the demographic features of new customers drawn so far by these
...
Chelsea's ethical code of conduct.
...
-   Are there any unusual differences in the quality of the data
    collected by MINGAR fitness wearable for customers of different
    races (particularly those with darker skin tones) and sleep
    patterns?

## Customers of MINGAR's New Afforable Lines

We used the consensus API to get 2016 median incomes for each census
...
The interpretability of our models is something that Naftali takes
pleasure in. The team makes a special effort to work with
...
Having said that, Naftali encountered a few restrictions during this
investigation. The team didn't have direct access to client income data
for the first study question, so they had to create estimates about a
customer's income based on their postal code. While this generality
...
With that stated, Naftali understands that income is sensitive
...
utilized the default skin tone. As a result, Naftali is unable to
establish the race/ethnicity of those users, which adds to the
...
When compiling and analyzing data, Naftali is committed to following
ethical guidelines for statistical practice. We vow to keep confidential
information, such as data on customers and devices private, without our
clients' permission. Furthermore, we want to draw careful conclusions
based on the facts and the research topics at hand. We have described
all of the assumptions we made when building the models, as well as any
potential limitations to our research, in this report, so that the
audience is fully informed about the context surrounding our
conclusions. Finally, we have a social obligation. The purpose of this
inquiry was not simply to see if products could have harmful biases
against particular groups, but also to make it simple for the general
public to comprehend how we came to those conclusions.
...
devices. Naftali initially checked the website's scraping permissions,
noting that we are authorized to scrape with a minimum of 12 seconds
between requests. We also gave details about our user agent to clarify
our intentions with the data we gathered. Only the information that was
strictly necessary was stored from the webpage. Our goal was to avoid
duplicating the dataset for this study and instead focus on factors that
were important to it.

## Accessing Census data on median household income

The Canadian census API was used to obtain data on median household
income. Naftali used CensusMapper to register an account, followed the
API's retrieval criteria, and cited CensusMapper in our report. This is
public data that is exposed to the public. The use of this data complies
with the Open Data Licence rules established by Statistics Canada.

## Accessing postcode conversion files

Census Canada postal code conversion data was obtained through the
University of Toronto portal. Because Naftali is made up of University
of Toronto students, they had the necessary credentials to access the
information. The Chelsea squad has consented to the license agreement by
downloading the dataset. In this report, the source for this information
has been credited.
